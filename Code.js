...
function getSheet() {
  var worksheetId = PropertiesService.getScriptProperties().getProperty("WORKSHEET_ID");
...
function createNotFoundTemplate() {
  return HtmlService.createTemplate("<h1>Not Found</h1><p>Response not found for ID <code><?= id ?></code></p>");
}

function createFormTemplate() {
  var template = HtmlService.createTemplate('<form action="<?= postURL ?>" target="_top" method="POST"><input type="hidden" name="id" value="<?= id ?>"><input type="submit" value="<?= label ?>"></form>');
